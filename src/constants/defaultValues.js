...
const widgetGridCode = `<script src="http://amusingly-widget-sync-api.s3-website.us-east-2.amazonaws.com/js/sync.api.js"></script><div id="amusing-widget-container" data-key="{data-key}"></div>`;
const widgetSliderCode = `<script src="http://amusingly-widget-sync-api.s3-website.us-east-2.amazonaws.com/js/sync.api.slider.js"></script><div id="am-slider-widget-container" data-key="{data-key}"></div>`;

export const widgetCode = (widgetType, key) => {
    let resCode = '';
    
    if (widgetType === 'Slider') {
        resCode = widgetSliderCode;
    } else {
        resCode = widgetGridCode;
    }

    return resCode.replace('{data-key}', key);
}